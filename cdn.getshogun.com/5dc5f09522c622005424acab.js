...
;(function($) {
  ...
  var shopify = {
    getProduct: function(meta, callback) {
      ...
      var url = "/products/" + meta.productHandle + ".js"
      $.ajax({
        url: url,
        method: "GET",
        dataType: "json"
    ...
    getInitialVariant: function(product) {
      var initVariantId = getParameterByName("variant")
      return product.variants.find(function(variant) {
        return String(variant.id) === initVariantId
      })
    }
  }
  ...
  var platforms = { shopify: shopify, big_commerce: defaultPlatform, commerce_cloud: defaultPlatform }
  ...
  function initProduct(meta, product) {
    ...
    updateProps({
      meta: meta,
      product: product,
      variant: product.initialVariant,
  ...
  function mapProduct(meta, product) {
    ...
    function initialVariant() {
      var initialVariant = platform.getInitialVariant && platform.getInitialVariant(product)
    ...
    product.initialVariant = initialVariant()
...
; (function (root, global, $) {
  ...
  function initImage() {
    ...
    if (imageSrcAttributes.src === PLACEHOLDER_IMAGE_SRC) $imageWrapper.addClass("shg-product-img-placeholder")

    setSrcAttributeOnImage(imageSrcAttributes)
   ...
   * @param {any} opts
   */
  function setSrcAttributeOnImage(opts) {
    var src = opts.src
    ...
    if (platformName !== SHOPIFY_PLATFORM) return
  ...
  function initImageZoom() {
    ...
    $imageZoom.children(".shg-image-zoom-background").attr("data-shg-lightbox-trigger", true)
  }
  ...
  function zoom($el, opts) {
    ...
    var backgroundImageSelector = "." + backgroundImageClass
    var src = opts.src || opts.image
    var scale = opts.scale || DEFAULT_ZOOM_SCALE
    var backgroundImageTemplate = '<div class="' + backgroundImageClass + '"></div>'
    ...
    $el
      .attr("data-shg-zoom-initialized", "true")
      .css({ maxWidth: width, maxHeight: height })
      .append(backgroundImageTemplate)
      .children(backgroundImageSelector)
    ...
    function addListeners(events) {
      $el
        .on(events.start, function (e) {
          e.preventDefault()
          $el.children(backgroundImageSelector).css({
        ...
        .on(events.move, function (e) {
          ...
          var clientX = e.pageX || (e.changedTouches ? e.changedTouches[0].pageX : 0)
          var clientY = e.pageY || (e.changedTouches ? e.changedTouches[0].pageY : 0)
          var x = clamp(((clientX - offset.left) / width) * 100, minOrigin, maxOrigin)
          var y = clamp(((clientY - offset.top) / height) * 100, minOrigin, maxOrigin)
          $el.children(backgroundImageSelector).css({
        ...
        .on(events.end, function (e) {
          e.preventDefault()
          $el.children(backgroundImageSelector).css({ transform: "" })
        })
    }
  }
  ...
  function attachEvents(callback) {
    ...
    function onFirstEvent(event) {
      ...
      callback(event.type === mouseEvents.start ? mouseEvents : touchEvents)
    }
  }
  ...
  function renderProductImage(props, queryComponentElement) {
    ...
    if (!$imageZoom.length || ($imageLink.length && window.SHOGUN_BROWSER.supportOnlyTouch))
      return
...
;(function($, SHGMoney) {
  ...
  window.__shgProductComponent.price = function(props, $data) {
    ...
    var config = props.meta.moneyFormat
    var currency = props.variant && props.variant["price_currency"]
    ...
    function priceContent(price) {
      ...
      var formatter = new window.SHGMoney(props.meta.platform, config)
...
;(function($) {
  ...
  window.__shgProductComponent.addToCart = function(props, $data, $element, $root) {
    ...
    function submitClickHandler() {
      ...
      if (props.meta.behavior !== "stay") return $form.find(":submit").click()
